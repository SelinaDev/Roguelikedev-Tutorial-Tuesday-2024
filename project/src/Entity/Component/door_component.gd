		...
		"close":
			if open:
				if not _parent_entity.map_data\
					.get_entities_at(PositionComponent.get_entity_position(_parent_entity))\
					.filter(func(e: Entity) -> bool: return e != _parent_entity)\
					.is_empty():
					Log.send_log(
						"%s cannot close because there is something in the way." % _parent_entity.get_entity_name(),
